...
Wheel::Wheel(int x, int y, int radius)
    : x(x), y(y), m_radius(radius) {}
...
void Wheel::simulate(const QList<Line>& lines, bool accelerating, bool braking) {
    ...
    for (const Line& line : lines) {
        ...
        if (dist < m_radius && intersection_x >= minX && intersection_x <= maxX) {
            ...
            // Response
            vNormalToLine *= RESTITUTION; // bounce with energy loss
            vAlongLine    *= FRICTION;    // surface friction

            // Player input along the ground tangent
    ...
    // Spring-damper constraints between attached wheels
    for (int i = 0; i < m_others.size(); ++i) {
        ...
        // Apply equal and opposite forces with damping
...
std::optional<std::array<int, 3>> Wheel::get(int x1, int y1, int x2, int y2, int cx, int cy) const {
    if (x + m_radius + cx < x1 || x - m_radius + cx > x2 || y + m_radius + cy < y1 || y - m_radius + cy > y2)
        return std::nullopt;
    return std::array<int, 3>{static_cast<int>(std::round(x + cx)), static_cast<int>(std::round(y + cy)), m_radius};
}
