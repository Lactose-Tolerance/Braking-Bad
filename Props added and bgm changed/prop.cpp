...
void PropSystem::maybeSpawnProp(int worldX, int groundGy, int levelIndex, std::mt19937& rng) {
    ...
    if (levelIndex == 0) { 
        ...
        if (chance < 0.015f)      m_props.append({worldX, wy, PropType::Igloo, varDist(rng), false});
        else if (chance < 0.030f) m_props.append({worldX, wy, PropType::Penguin, varDist(rng), (bool)flipDist(rng)});
        else if (chance < 0.045f) m_props.append({worldX, wy, PropType::Snowman, varDist(rng), (bool)flipDist(rng)});
        else if (chance < 0.070f) m_props.append({worldX, wy, PropType::IceSpike, varDist(rng), (bool)flipDist(rng)});
        else if (chance < 0.090f) m_props.append({worldX, wy, PropType::DeadTree, varDist(rng), (bool)flipDist(rng)});
        else if (chance < 0.095f) m_props.append({worldX, wy, PropType::Signpost, varDist(rng), (bool)flipDist(rng)});
    }
        ...
        else if (chance < 0.025f) m_props.append({worldX, wy, PropType::Alien, varDist(rng), false});
    }
}

void PropSystem::draw(QPainter& p, int camX, int camY, int screenW, int screenH) {
    ...
    for (const Prop& prop : m_props) {
        ...
        switch (prop.type) {
        case PropType::Tree:       drawTree(p, gx, gy, prop.wx, prop.wy, prop.variant); break;
        ...
        case PropType::Igloo:      drawIgloo(p, gx, gy, prop.variant); break;
        ...
        case PropType::DeadTree:   drawDeadTree(p, gx, gy, prop.variant); break;
        case PropType::Signpost:   drawSignpost(p, gx, gy, prop.variant); break;
        case PropType::UFO:        drawUFO(p, gx, gy, prop.variant); break;
        case PropType::Rover:      drawRover(p, gx, gy, prop.variant, prop.flipped); break;
...
void PropSystem::drawTree(QPainter& p, int gx, int gy, int wx, int wy, int variant) {
    ...
    for(int y = 0; y <= trunkH; y++) {
        ...
        plot(p, gx - trunkW/2, drawY, cTrunkDark);
        
        plot(p, gx + trunkW/2, drawY, cTrunkDark);

        
        for(int x = -trunkW/2 + 1; x < trunkW/2; x++) {
            plot(p, gx+x, drawY, cTrunk);
            
            if (x == -1 && y > 0 && y % 3 == 0) plot(p, gx+x, drawY, cTrunkDark);
            if (x == 1 && y > 0 && y % 4 == 0) plot(p, gx+x, drawY, cTrunk.lighter(110));
        }

        
        if (y >= 0 && y <= 1) {
            plot(p, gx - trunkW/2 - 1, drawY, cTrunkDark);
            plot(p, gx + trunkW/2 + 1, drawY, cTrunkDark);
        }
    }
    
    plot(p, gx, gy - trunkH/2, cHole);
    plot(p, gx, gy - trunkH/2 - 1, cHole);

    
    int folBot = gy - trunkH + 2;
...
void PropSystem::drawIgloo(QPainter& p, int gx, int gy, int variant) {
    ...
    for(int dy = -r; dy <= 0; dy++) {
        for(int dx = -r; dx <= r; dx++) {
            if (dx*dx + dy*dy <= r*r) {
                bool shadow = (dx > r/2) || (dy > -r/4);
                plot(p, gx+dx, gy+dy, shadow ? iceShadow : ice);
            }
        }
    }
    for(int y=1; y<=3; y++) {
        for(int dx = -r + 1; dx <= r - 1; dx++) plot(p, gx+dx, gy+y, iceShadow);
    }
    int tunW = 6, tunH = 7;
    for(int y=0; y<tunH; y++) {
        for(int x=-tunW; x<=tunW; x++) plot(p, gx+x, gy-y, iceShadow);
    }
    for(int y=1; y<=3; y++) {
        for(int x=-tunW; x<=tunW; x++) plot(p, gx+x, gy+y, iceShadow);
    }
    for(int y=0; y<tunH-2; y++) {
        for(int x=-3; x<=3; x++) plot(p, gx+x, gy-y, dark);
    }
}
...
void PropSystem::drawDeadTree(QPainter& p, int gx, int gy, int variant) {
    QColor wood(80, 70, 60);
    QColor snow(240, 240, 250);
    int h = 10 + variant;
    for(int y=0; y<h; y++) plot(p, gx, gy-y, wood);
    plot(p, gx-1, gy-4, wood); plot(p, gx-2, gy-3, wood);
    plot(p, gx+1, gy-6, wood); plot(p, gx+2, gy-5, wood);
    plot(p, gx, gy-h, snow);
    plot(p, gx-2, gy-4, snow);
}

void PropSystem::drawSignpost(QPainter& p, int gx, int gy, int variant) {
    QColor wood(120, 80, 40);
    QColor sign(200, 180, 150);
    for(int y=0; y<8; y++) plot(p, gx, gy-y, wood);
    for(int y=6; y<9; y++) {
        for(int x=-2; x<=2; x++) plot(p, gx+x, gy-y, sign);
    }
}
...
void PropSystem::drawUFO(QPainter& p, int gx, int gy, int variant) {
    ...
    plot(p, gx, gy-2, glass);
    plot(p, gx-1, gy-2, glass);
    plot(p, gx+1, gy-2, glass);
    plot(p, gx, gy-3, glass);
...
void PropSystem::drawRover(QPainter& p, int gx, int gy, int variant, bool flipped) {
    int d = flipped ? -1 : 1;
    QColor wheelC(30, 30, 35); QColor chassisC(220, 220, 220);
    QColor detailC(50, 50, 60); QColor lensC(20, 30, 80); QColor gold(200, 170, 50);
    int wheelY = gy;
    auto drawWheel = [&](int wx, int wy) {
        plot(p, wx, wy, wheelC); plot(p, wx+1, wy, wheelC);
        plot(p, wx, wy-1, wheelC); plot(p, wx+1, wy-1, wheelC);
    };
    drawWheel(gx - (5*d), wheelY); drawWheel(gx - (1*d), wheelY); drawWheel(gx + (5*d), wheelY);
    plot(p, gx-(5*d), wheelY-2, detailC); plot(p, gx-(1*d), wheelY-2, detailC); plot(p, gx+(5*d), wheelY-2, detailC);
    int bodyY = gy - 3;
    for(int x=-6; x<=6; x++) { plot(p, gx+(x*d), bodyY, chassisC); plot(p, gx+(x*d), bodyY-1, chassisC); }
    plot(p, gx-(5*d), bodyY-2, detailC); plot(p, gx-(6*d), bodyY-2, detailC); plot(p, gx-(5*d), bodyY-3, detailC);
    int mastX = gx + (4*d);
    plot(p, mastX, bodyY-2, detailC); plot(p, mastX, bodyY-3, detailC); plot(p, mastX, bodyY-4, detailC);
    plot(p, mastX, bodyY-5, chassisC); plot(p, mastX+(1*d), bodyY-5, lensC);
    int dishX = gx - (1*d);
    plot(p, dishX, bodyY-2, detailC);
    plot(p, dishX-1, bodyY-3, gold); plot(p, dishX, bodyY-3, gold); plot(p, dishX+1, bodyY-3, gold);
    plot(p, dishX-2, bodyY-4, gold); plot(p, dishX+2, bodyY-4, gold);
}
